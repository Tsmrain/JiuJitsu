...
Se propone el desarrollo e implementación de un sistema computacional de asistencia al entrenamiento basado en visión artificial, concebido para comparar la ejecución técnica del practicante con un video de referencia provisto por el instructor. El sistema opera de manera similar a un **espejo inteligente**: al igual que un espejo que sobrepone de forma virtual la técnica correcta sobre el reflejo del practicante, el software resalta de inmediato en la pantalla las partes del cuerpo desalineadas respecto al modelo del instructor. El procesamiento de video se realiza en tiempo diferido y genera reportes visuales con indicadores claros y directos sobre la imagen. Esta retroalimentación objetiva y constante está disponible para el practicante a través de su teléfono celular, facilitando el autoaprendizaje guiado y liberando tiempo para que el instructor concentre su labor pedagógica en correcciones tácticas y estratégicas avanzadas.
...
* **Delimitación temática y técnica:** El sistema operará con un enfoque agnóstico de comparación técnica: procesará cualquier técnica de artes marciales siempre que se disponga de un video de referencia del instructor y un video de ejecución del practicante bajo un protocolo de grabación establecido. La estimación postural se enfocará estrictamente en la extracción de puntos clave articulares (*keypoints*) en dos dimensiones (2D) y el análisis de variaciones angulares relativas a lo largo del tiempo. El sistema constituye una herramienta complementaria de autoevaluación y auditoría técnica, sin pretender sustituir el juicio pedagógico del instructor. Se excluyen del alcance el reconocimiento automático o clasificación de técnicas no catalogadas, la reconstrucción volumétrica tridimensional (3D) y la medición de variables biomecánicas de fuerza, potencia o fatiga física.
...
5. **El Cuello de Botella Crítico:**
   ...
   * Si un practicante comete un error biomecánico en ese intervalo, lo repite varias veces hasta que el instructor llega, fijando el vicio motor.
...
**Justificación de invarianza:** La formulación vectorial asegura invarianza matemática frente a traslaciones en el plano y variaciones de escala geométrica. Por consiguiente, divergencias en la distancia focal o posición relativa de los practicantes respecto a la cámara no alteran la estimación angular, facultando una comparación directa y robusta entre el video del instructor y el del practicante. El sistema utiliza la técnica del instructor como una **plantilla de referencia**: compara la postura del practicante con este modelo ideal para identificar con precisión qué articulaciones no coinciden con la ejecución correcta.
...
```mermaid
sequenceDiagram
    participant App as Celular del Practicante (PWA)
    participant Edge as Servidor Local en Tatami (FastAPI)
    participant Drive as Almacenamiento Intermedio en la Nube (Google Drive)
    participant Colab as Servidor en la Nube (Google Colab Pro)

    App->>Edge: Carga de Video desde el Celular
    Edge->>Drive: Publicación de archivo (ID_PRACTICANTE_TECNICA_FECHA.mp4)
...
1. **Almacenamiento Intermedio en la Nube (Google Drive):** El almacenamiento compartido evita la necesidad de conexiones continuas complejas (como túneles o WebSockets), las cuales suelen fallar en las redes Wi-Fi de gimnasios.
2. **Procesamiento en Segundo Plano:** El servidor local funciona de forma liviana, encargándose únicamente de recibir el video y enviarlo a la nube. El procesamiento pesado se ejecuta en segundo plano en Google Colab Pro mediante aceleradores gráficos (GPU). El sistema compara la postura del practicante con el modelo de referencia del instructor y dibuja los indicadores visuales cuando detecta errores. El resultado queda guardado para su consulta en cualquier momento, evitando sobrecargar la computadora del gimnasio o el celular del practicante.
...
1. **Gestión de Técnicas Maestras:** Permite al Instructor registrar, etiquetar y homologar videos de referencia canónica demostrados en el tatami.
2. **Gestión de Materiales de Estudio:** Permite al Instructor subir libros y manuales en formato PDF (como *Jiu-Jitsu University*) y registrar enlaces a videos oficiales de YouTube, sirviendo como base de conocimiento para la IA y material de consulta para los practicantes.
3. **Carga de Videos desde el Celular:** Facilita a los Practicantes seleccionar la técnica del día y subir grabaciones breves de su práctica en pareja (clips de hasta 6 segundos y 5 MB) directamente desde su teléfono móvil.
4. **Extracción y Aislamiento Corporal Automatizado:** Identifica los puntos clave del cuerpo (*keypoints*) mediante **YOLO26-Pose** y separa de forma automática al Practicante activo de su compañero de apoyo.
5. **Sincronización y Comparación Postural:** Alinea los tiempos de ejecución mediante **DTW** (*Dynamic Time Warping*) y compara la postura del Practicante con la técnica del Instructor, utilizándola como una plantilla de referencia.
...
* **Similitud de Postura / Coincidencia de Posición:** Nivel de coincidencia entre la posición del cuerpo del Practicante y el molde de referencia del Instructor en una fase técnica equivalente.
* **DTW (*Dynamic Time Warping* / Sincronizador de Movimiento):** Método que empareja movimientos que ocurren a diferente velocidad, permitiendo comparar la técnica aunque el Practicante se mueva más despacio o haga pausas.
* **YOLO26-Pose:** Modelo de visión artificial que detecta cuerpos y extrae los puntos articulares de forma rápida y precisa.
* **Google Gemini API:** Servicio de inteligencia artificial de Google que analiza los desajustes posturales y redacta recomendaciones pedagógicas en lenguaje claro.
...
1. **Capa Frontal en el Tatami (PWA Móvil):** Funciona en los teléfonos celulares del Instructor y de los Practicantes. Permite consultar el catálogo técnico, cargar videos de práctica y revisar los diagnósticos visuales y los consejos pedagógicos.
2. **Capa de Procesamiento en la Nube (Google Colab Pro + FastAPI + Gemini API):** Servicio centralizado que recibe los videos, extrae las posiciones corporales con YOLO26, sincroniza los tiempos con DTW, analiza la coincidencia postural y consulta la API de Google Gemini para redactar las recomendaciones de mejora.
...
```mermaid
flowchart TD
    A[El Instructor registra Técnica Maestra y Materiales] --> B[El Practicante selecciona técnica y sube video desde el celular]
    ...
    D --> E[El sistema evalúa coincidencia postural contra la plantilla]
    ...
    G --> H
```
*Nota.* Flujo secuencial de procesamiento en segundo plano para asistencia en el tatami.

1. **Gestión de Técnicas Maestras:** Registro del video patrón por parte del Instructor.
2. **Gestión de Materiales de Estudio:** Subida de libros en PDF y registro de enlaces a videos oficiales de YouTube por parte del Instructor.
3. **Carga de Videos desde el Celular:** Subida rápida desde el teléfono móvil con control de duración (< 6s) y tamaño (< 5 MB).
4. **Aislamiento del Practicante Activo:** Separación automática del compañero que actúa como soporte estático.
5. **Sincronización Temporal (DTW):** Comparación justa de movimientos ejecutados a distintas velocidades.
6. **Evaluación de Coincidencia Postural:** Comparación con la técnica del Instructor usada como plantilla de referencia.
7. **Anotación Visual:** Marcado de un círculo rojo en el fotograma clave sobre la parte del cuerpo desalineada.
...
* **Rol y Responsabilidades:** Máxima autoridad técnica y pedagógica en la academia. Diseña el programa de clases, demuestra las técnicas oficiales, sube los materiales de estudio (PDFs y enlaces de YouTube) y evalúa los avances para los ascensos de cinturón.
* **Contexto Operativo:** Supervisa clases con múltiples parejas entrenando al mismo tiempo. Necesita una herramienta que le permita establecer modelos de referencia oficiales para que los practicantes puedan autoevaluarse mientras asiste a otras parejas de practicantes.
...
* **Necesidades Principales:** Registrar de forma rápida los videos canónicos de cada técnica y compartir material de apoyo confiable con los practicantes.
...
* **Contexto Operativo:** Asiste a las clases regulares y requiere saber si sus posturas y apoyos son correctos, especialmente si ha dejado de entrenar por algún tiempo y necesita refrescar los movimientos.
* **Competencia Digital:** Nivel regular o avanzado; usuario habitual de teléfonos inteligentes. Espera respuestas rápidas, bajo consumo de datos y explicaciones fáciles de entender.
...
* **Almacenamiento Intermedio en la Nube (Google Drive Storage):** Repositorio en la nube para guardar de forma temporal y segura los archivos de video, diagnósticos y reportes.
...
| :---: | :--- | :--- |
| **RF-01** | **Registro de Técnica Maestra** | **Como** Instructor,<br>**se requiere** registrar y homologar el video patrón canónico de la técnica oficial (ej. *'Escape de Montada mediante Puente y Giro'*),<br>**para que** sirva como la plantilla de referencia contra la cual se evaluará la ejecución técnica de los practicantes.<br>*Criterio de Aceptación:* El sistema permite cargar el video patrón, asociar los metadatos de categoría y posición de origen en menos de 30 segundos, extrayendo y almacenando el esqueleto de referencia en el servidor remoto. |
| **RF-02** | **Carga de Videos desde el Celular** | **Como** Practicante,<br>**el sistema debe** permitir seleccionar desde el teléfono móvil la técnica demostrada en la sesión y subir la grabación de su práctica en pareja (duración de 4 a 6 segundos),<br>**para que** el sistema realice la evaluación postural.<br>*Criterio de Aceptación:* La interfaz valida que el archivo no supere 5 MB de tamaño ni 6 segundos de duración, interrumpiendo la transferencia antes de saturar el enlace si se exceden dichos parámetros. |
...
| **RF-11** | **Subida de Libros y Manuales (PDF)** | **Como** Instructor,<br>**se requiere** subir archivos PDF de libros y manuales de Jiu-Jitsu (como *Jiu-Jitsu University*),<br>**para que** el sistema lea estos libros, disponga de información técnica correcta y la utilice al formular los consejos para los practicantes.<br>*Criterio de Aceptación:* El sistema permite cargar archivos PDF, indexa su contenido técnico y lo vincula a las técnicas correspondientes como base de conocimiento oficial para la inteligencia artificial. |
| **RF-12** | **Enlaces a Videos de YouTube** | **Como** Instructor,<br>**se requiere** guardar enlaces de videos oficiales de YouTube vinculados a cada técnica,<br>**para que** los Practicantes dispongan de ejemplos adicionales de consulta cuando se preparan para sus exámenes de cinturón.<br>*Criterio de Aceptación:* El sistema valida el formato del enlace de YouTube, lo guarda en el catálogo de la técnica maestra y permite su reproducción directa desde la aplicación. |
...
1. **El Instructor:** Usuario docente responsable de registrar las técnicas maestras oficiales, subir manuales en PDF (como *Jiu-Jitsu University*), registrar enlaces de YouTube y supervisar la progresión técnica en el tatami.
...
3. **Servicios en la Nube:** Componente computacional externo que ejecuta la inferencia esquelética con YOLO26, sincroniza las secuencias temporales mediante DTW, evalúa la coincidencia postural y sintetiza las recomendaciones mediante Google Gemini.
...
graph LR
    ...
    subgraph Sistema["Sistema de Asistencia Postural BJJ"]
        ...
        CU02["CU-02: Cargar Video desde el Celular"]
        CU03["CU-03: Visualizar Diagnóstico y Consejo de IA"]
        CU04["CU-04: Consultar Historial de Progreso"]
        CU05["CU-05: Gestionar Materiales de Estudio (PDF y YouTube)"]
    end
...
| :---: | :--- | :---: | :---: | :--- |
| **CU-01** | Registrar Técnica Maestra | El Instructor | RF-01, RF-03 | El Instructor graba y sube el video de referencia canónica. El sistema procesa los puntos corporales del modelo y lo guarda en el catálogo. |
| **CU-02** | Cargar Video desde el Celular | El Practicante | RF-02, RF-03, RF-04, RF-05, RF-09 | El Practicante selecciona la técnica y sube su clip (< 6s). El sistema filtra al compañero de apoyo, sincroniza los tiempos con DTW y verifica que la toma sea válida. |
| **CU-03** | Visualizar Diagnóstico y Consejo de IA | El Practicante | RF-06, RF-07, RF-08, RP-01, RP-02 | El sistema muestra la imagen clave con un círculo rojo en la articulación desalineada y el consejo pedagógico generado por Google Gemini en menos de 5 segundos. |
...
| **CU-05** | Gestionar Materiales de Estudio | El Instructor / El Practicante | RF-11, RF-12 | El Instructor sube manuales en PDF y guarda enlaces de YouTube. El Practicante los consulta como material de estudio para sus exámenes de cinturón. |
...
* **Instructor:** Especialización de Usuario que representa al Instructor Titular. Posee permisos exclusivos para registrar y homologar Técnicas Maestras, así como para subir Materiales de Estudio (PDFs y enlaces de YouTube).
...
* **TecnicaMaestra:** Modela la ejecución canónica demostrada por el Instructor. Incluye el nombre de la técnica (ej. *'Escape de Montada'*), su categoría, la posición inicial y el video patrón con sus puntos articulares de referencia.
* **MaterialEstudio:** Modela los recursos de apoyo subidos por el Instructor, incluyendo manuales y libros en formato PDF (como *Jiu-Jitsu University*) y enlaces a videos de YouTube. Sirve como base de conocimiento para enriquecer los consejos de la inteligencia artificial y como material de consulta para los practicantes.
* **VideoPractica:** Registro en video grabado por el Practicante junto a su compañero desde el celular, con duración menor a 6 segundos y peso inferior a 5 MB.
