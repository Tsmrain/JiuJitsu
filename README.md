...
| **RF-11** | Rechazo por Oclusión Prolongada y Protección de Integridad de Datos | El sistema deberá interrumpir el cómputo del diagnóstico cuando un tramo de oclusión continua supere el umbral máximo de validez definido en el RF-08, notificando al estudiante mediante un mensaje explícito en pantalla ("No fue posible calcular el diagnóstico: oclusión prolongada de la articulación durante la ejecución. Vuelve a grabar con mejor ángulo de cámara.") en lugar de renderizar fotogramas con datos inexactos, abortando la ejecución a nivel de base de datos (sin persistir registros en las tablas `AnalisisBiomecanico` ni `HistorialProgresion`) para evitar contaminar el historial de progresión del atleta con cinemáticas ficticias. *(Nota de Arquitectura Financiera: Si bien el aborto transaccional evita la persistencia de datos erróneos, **no elimina el costo de cómputo** de los milisegundos ya consumidos por la inferencia de RTMPose hasta el punto de detección de la oclusión. Esta decisión de diseño prioriza la **validez pedagógica y la integridad estadística** del historial longitudinal del atleta (evitando contaminación de datos) sobre el ahorro marginal de recursos de CPU. La trazabilidad de estos costos 'huérfanos' se delega a las métricas de AOM (Application Operations Management) de Huawei Cloud, manteniendo la base de datos relacional limpia).* |
