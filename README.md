...
| Volumen de Datos Requerido | 25% | 5 (Cero entrenamiento supervisado / One-Shot) | 5 (Cero muestras) | 1 (Exige miles de muestras) |
...
*Aclaración Metodológica sobre Aprendizaje y Parametrización:* El sistema no requiere un dataset masivo etiquetado para entrenar redes neuronales (como sí lo demandaría un clasificador de acciones o secuencias motoras). La "calibración" realizada por el Head Coach consiste estrictamente en la parametrización de reglas de negocio pedagógicas (umbrales angulares tolerados y mensajes causales explicativos), no en el entrenamiento estadístico de los algoritmos de sincronización. Bajo este enfoque de *One-Shot Learning*, el sistema asimila e implementa de forma instantánea cualquier técnica deportiva novedosa a partir de un único video patrón de referencia.
...
La **Figura 5.6** presenta el DCD consolidado del sistema. En concordancia con los contratos de integridad referencial y de dominio (Larman / Mannino), la clase `VideoEjecucion` incorpora en su método `validarLimites(): Boolean` un truncamiento matemático estricto a dos decimales (`round(peso_bytes / 1024^2, 2)`) previo a la persistencia. Esta salvaguarda garantiza consistencia transaccional y previene desbordamientos de escala o excepciones numéricas frente a la restricción relacional `CHECK (peso_mb <= 5.0)` en PostgreSQL:
