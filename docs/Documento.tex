...
\textbf{CU-04: Visualizar Analítica de Tatami (Profesor)}
\begin{enumerate}
    \item El \textbf{Profesor} solicita listar las técnicas más practicadas.
    \item El \textbf{Sistema} retorna el top de técnicas con más evaluaciones.
    \item El \textbf{Profesor} selecciona una técnica específica.
    \item El \textbf{Sistema} consolida las evaluaciones históricas y devuelve el total, porcentaje de aprobación y un detalle con la cantidad de fallas por articulación (\texttt{total\_detecciones}) y su desvío promedio.
\end{enumerate}

\subsection{Contratos de Operación}
\begin{itemize}
    \item \textbf{Operación}: \texttt{obtenerAnaliticaTecnica(id\_tecnica)}
    \item \textbf{Referencias Cruzadas}: CU-04 (RF-20).
    \item \textbf{Precondiciones}: Existen evaluaciones guardadas para la técnica.
    \item \textbf{Postcondiciones}: Se retornó un resumen estadístico consolidando todas las instancias de \texttt{EvaluacionAlumno} asociadas a \texttt{id\_tecnica}, agrupando las fallas de \texttt{desviaciones\_detalle} por nombre de articulación.
\end{itemize}

\subsection{Modelo de Diseño (Interacciones y DCD)}
Para soportar el CU-04, se aplicaron los patrones GRASP y la arquitectura en 5 capas:
\begin{itemize}
    \item \textbf{Controller}: Se creó \texttt{AnaliticaController} que recibe los pedidos de analítica de la API y orquesta la consulta de datos.
    \item \textbf{Pure Fabrication / Repository}: Se definió la interfaz \texttt{IHistorialRepository} y su implementación concreta con la responsabilidad cohesiva de ejecutar la agregación de datos y cálculos estadísticos en la base de datos.
    \item \textbf{Information Expert}: El repositorio cuenta con la información para desdoblar el JSONB (\texttt{jsonb\_array\_elements}) y sumar las \texttt{total\_detecciones} para retornarlas estructuradas al controlador y de ahí al Frontend PWA.
\end{itemize}
